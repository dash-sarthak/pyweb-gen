...
Any safety fact you can't get to step 4, say so out loud. Don't write it up as settled. Step 4 is usually one small script or test that imports the same package and calls the exact function you're worried about.

## Steps

1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Pull the originating commits and PR/ticket via `git log`, `git blame`, and the tracker if one is linked, for the intent behind the change.
...
3. Look where grep stops. Read the source of the library you call, and check its pinned version and any local patch. Work out when things run: goroutines, deferred cleanup, context cancellation, retries. Follow what a symbol search misses: the bytes on the wire (gRPC/proto schema), a config file, another service reading the same data, a feature flag, code three hops downstream.
...
6. For a big or wide change, get a second independent pass: a fresh subagent (no context from this investigation) reviewing the same diff cold catches different things than continuing to reason from the same thread.
...
Write it through the `unslop` skill if it's available, cite real code, and strip anything private before it goes anywhere public.
